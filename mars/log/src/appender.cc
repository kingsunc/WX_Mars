...
static void __appender_sync(const XLoggerInfo* _info, const char* _log)
{
    ...
    AutoBuffer tmp_buff;
	if (!sg_log_buff->Write(log.Ptr(), log.Length(), tmp_buff))
	{
		return;
	}
...
static void __appender_async(const XLoggerInfo* _info, const char* _log)
{
	...
	if (!sg_log_buff->Write(log_buff.Ptr(), (unsigned int)log_buff.Length()))
	{
		return;
	}
...
const char* xlogger_dump(const void* _dumpbuffer, size_t _len)
{
    ...
    snprintf(file_name, sizeof(file_name), "%d%02d%02d%02d%02d%02d_%d.dump", 1900 + tcur.tm_year, 1 + tcur.tm_mon, tcur.tm_mday, tcur.tm_hour, tcur.tm_min, tcur.tm_sec, (int)_len);
    filepath += file_name;
