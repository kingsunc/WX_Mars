...
void SimpleIPPortSort::__RemoveTimeoutXml()
{
    ...
    for (std::vector<tinyxml2::XMLElement*>::iterator iter = remove_ele_ptr_list.begin(); iter != remove_ele_ptr_list.end(); ++iter)
	{
...
void SimpleIPPortSort::InitHistory2BannedList(bool _savexml)
{
    ...
    for (record = recordsxml_.FirstChildElement(kRecord); NULL != record; record = record->NextSiblingElement(kRecord))
	{
