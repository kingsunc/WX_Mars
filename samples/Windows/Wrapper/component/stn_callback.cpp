...
{
    NetworkService::GetInstance().OnPush(_channel_id, _cmdid, _taskid, _body, _extend);
}
...
{
	return NetworkService::GetInstance().Req2Buf(_taskid, _user_context, _outbuffer, _extend, _error_code, _channel_select);
}
...
{
	return NetworkService::GetInstance().Buf2Resp(_taskid, _user_context, _inbuffer, _extend, _error_code, _channel_select);
}

int StnCallBack::OnTaskEnd(uint32_t _taskid, void* const _user_context, int _error_type, int _error_code)
{
	NetworkService::GetInstance().OnTaskEnd(_taskid, _user_context, _error_type, _error_code);
	return 0;
}
