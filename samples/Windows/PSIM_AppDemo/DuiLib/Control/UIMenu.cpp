...
namespace DuiLib {
	...
	LRESULT CMenuWnd::OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		if( m_pOwner != NULL) {
			...
			CControlUI* pRoot = builder.Create(m_xml,UINT(0), &m_pm);
	...
	void CMenuWnd::ResizeMenu()
	{
		...
		int cxFixed = rc.GetWidth();
		int cyFixed = rc.GetHeight();
		...
		if (rc.bottom + SHADOW_WIDTH > rcWork.bottom)
		{
			rc.bottom = rc.top;
			rc.top = rc.bottom - cyFixed;
		}

		if (rc.right + SHADOW_WIDTH > rcWork.right)
		{
			rc.right = rc.left;
			rc.left = rc.right - cxFixed;
		}

		if (rc.top - SHADOW_WIDTH < rcWork.top)
		{
			rc.top = rc.bottom;
			rc.bottom = rc.top + cyFixed;
		}

		if (rc.left - SHADOW_WIDTH < rcWork.left)
		{
			rc.left = rc.right;
	...
	void CMenuWnd::ResizeSubMenu()
	{
		...
		rc.left = rcWindow.right + SHADOW_WIDTH;
		rc.right = rc.left + cxFixed;
		rc.right += 2;
		...
		cxFixed -= SHADOW_WIDTH;
		MoveWindow(m_hWnd, rc.left, rc.top, cxFixed, rc.bottom - rc.top + m_pLayout->GetInset().top + m_pLayout->GetInset().bottom, FALSE);
	}
	...
	SIZE CMenuElementUI::EstimateSize(SIZE szAvailable)
	{
		...
		if(cXY.cy == 0) {
			...
			RECT rcText = { 0, 0, DUI_MAX(szAvailable.cx, m_cxyFixed.cx), 9999 };
			rcText.left += pInfo->rcTextPadding.left;
			rcText.right -= pInfo->rcTextPadding.right;
			if( pInfo->bShowHtml ) {   
				int nLinks = 0;
