...
namespace DuiLib {
	...
	void CComboWnd::Init(CComboUI* pOwner)
	{
		...
		rc.bottom = rc.top + DUI_MIN(cyFixed, szDrop.cy);
		...
		if( rc.bottom > rcWork.bottom ) {
			...
			rc.top = rcOwner.top - DUI_MIN(cyFixed, szDrop.cy);
	...
	bool CComboUI::SelectItem(int iIndex, bool bTakeFocus, bool bChanged)
	{
		if( iIndex == m_iCurSel ) return true;
		int iOldSel = m_iCurSel;
		if( m_iCurSel >= 0 )
	...
	bool CComboUI::SelectItemText(LPCTSTR strSelText)
	{
		int iCount = GetCount();
		for (int iIndex = 0; iIndex < GetCount(); ++iIndex)
		{
			...
			if (NULL != pItem)
			{
				CDuiString strTemp = pItem->GetText();
				if (0 == strTemp.Compare(strSelText))
				{
