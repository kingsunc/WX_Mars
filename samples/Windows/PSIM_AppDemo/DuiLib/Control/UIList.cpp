...
namespace DuiLib {
	...
	bool CListUI::Add(CControlUI* pControl)
	{
		...
		if( pControl->GetInterface(_T("ListHeader")) != NULL ) {
			...
			m_ListInfo.nColumns = DUI_MIN(m_pHeader->GetCount(), UILIST_MAX_COLUMNS);
	...
	void CListBodyUI::SetScrollPos(SIZE szPos, bool bMsg)
	{
		...
		if( cx != 0 && m_pOwner ) {
			...
			pInfo->nColumns = DUI_MIN(pHeader->GetCount(), UILIST_MAX_COLUMNS);
	...
	void CListBodyUI::SetPos(RECT rc, bool bNeedInvalidate)
	{
		...
		for( int it1 = 0; it1 < m_items.GetSize(); it1++ ) {
			...
			sz.cx = DUI_MAX(sz.cx, 0);
			if( sz.cx < pControl->GetMinWidth() ) sz.cx = pControl->GetMinWidth();
			if( sz.cx > pControl->GetMaxWidth() ) sz.cx = pControl->GetMaxWidth();
			cxNeeded = DUI_MAX(cxNeeded, sz.cx);
			nEstimateNum++;
		}
		...
		if( m_pOwner ) {
			...
			if( pHeader != NULL && pHeader->GetCount() > 0 ) {
				cxNeeded = DUI_MAX(0, pHeader->EstimateSize(CDuiSize(rc.right - rc.left, rc.bottom - rc.top)).cx);
				if ( m_pHorizontalScrollBar && m_pHorizontalScrollBar->IsVisible())
				{
		...
		int cyNeeded = 0;
		int cyExpand = 0;
		if( nAdjustables > 0 ) cyExpand = DUI_MAX(0, (szAvailable.cy - cyFixed) / nAdjustables);
		...
		for( int it2 = 0; it2 < m_items.GetSize(); it2++ ) {
			...
			if( sz.cy == 0 ) {
				...
				if( iAdjustable == nAdjustables ) {
					sz.cy = DUI_MAX(0, szRemaining.cy - rcPadding.bottom - cyFixedRemaining);
				} 
				if( sz.cy < pControl->GetMinHeight() ) sz.cy = pControl->GetMinHeight();
				if( sz.cy > pControl->GetMaxHeight() ) sz.cy = pControl->GetMaxHeight();
			}
			...
			sz.cx = DUI_MAX(cxNeeded, szAvailable.cx - rcPadding.left - rcPadding.right);
	...
	SIZE CListHeaderUI::EstimateSize(SIZE szAvailable)
	{
		...
		if( cXY.cy == 0 && m_pManager != NULL ) {
			for( int it = 0; it < m_items.GetSize(); it++ ) {
				cXY.cy = DUI_MAX(cXY.cy,static_cast<CControlUI*>(m_items[it])->EstimateSize(szAvailable).cy);
			}
			int nMin = m_pManager->GetDefaultFontInfo()->tm.tmHeight + 6;
			cXY.cy = DUI_MAX(cXY.cy,nMin);
		}
	...
	void CListHeaderUI::SetPos(RECT rc, bool bNeedInvalidate)
	{
		...
		int cxExpand = 0;
		int cxNeeded = 0;
		if( nAdjustables > 0 ) cxExpand = DUI_MAX(0, (szAvailable.cx - cxFixed) / nAdjustables);
		// Position the elements
		SIZE szRemaining = szAvailable;
		int iPosX = rc.left;
		...
		for( int it2 = 0; it2 < m_items.GetSize(); it2++ ) {
			...
			if( sz.cx == 0 ) {
				...
				// Distribute remaining to last element (usually round-off left-overs)
				if( iAdjustable == nAdjustables ) {
					sz.cx = DUI_MAX(0, szRemaining.cx - rcPadding.right - cxFixedRemaining);
				}
				if( sz.cx < pControl->GetMinWidth() ) sz.cx = pControl->GetMinWidth();
				if( sz.cx > pControl->GetMaxWidth() ) sz.cx = pControl->GetMaxWidth();
			}
