...
namespace DuiLib {
	...
	CDialogBuilder::CDialogBuilder() : m_pstrtype(NULL)
	{
		m_instance = NULL;
	}

	CControlUI* CDialogBuilder::Create(STRINGorID xml, LPCTSTR type, 
		CPaintManagerUI* pManager, CControlUI* pParent)
	{
		...
		return Create(pManager, pParent);
	}

	CControlUI* CDialogBuilder::Create(CPaintManagerUI* pManager, CControlUI* pParent)
	{
			...
			if( _tcsicmp(pstrClass, _T("Include")) == 0 ) 
			{
				...
				for ( int i = 0; i < count; i++ ) 
				{
					...
					{ // 使用资源dll，从资源中读取
						...
						pControl = builder.Create((UINT)id, m_pstrtype, pManager, pParent);
					}
					else 
					{
						pControl = builder.Create((LPCTSTR)szValue, (UINT)0, pManager, pParent);
					}
				}
				continue;
			}
