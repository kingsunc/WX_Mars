...
namespace DuiLib
{
	...
	void CContainerUI::ScrollToBottom()
	{
		...
		// 鼠标不在Container内时，自动滚动到底部;
		if (m_pVerticalScrollBar && (!::PtInRect(&rcContainer, pt)) )
		{
			int iTotalHeight = 0;
			for (int i = 0; i < GetCount(); i++)
			{
				...
				if (pItem)
				{
					iTotalHeight += pItem->GetFixedHeight();
				}
			}
			...
			sz.cx = 0;
			sz.cy = iTotalHeight;
			// 添加完毕以后，要等待一个消息循环后再调用SetScrollPos;
