...
namespace DuiLib {
	...
	bool CRenderEngine::DrawIconImageString(HDC hDC, CPaintManagerUI* pManager, const RECT& rc, const RECT& rcPaint, 
		...
		TDrawInfo& drawInfo)
	{
		...
		if( drawInfo.pImageInfo == NULL ) {
			...
			const TImageInfo* data = NULL;
			if(sImageResType.IsEmpty()) bUseRes = false;
	...
	void CRenderEngine::DrawHtmlText(HDC hDC, CPaintManagerUI* pManager, RECT& rc, LPCTSTR pstrText, DWORD dwTextColor, RECT* prcLinks, CDuiString* sLinks, int& nLinkRects, UINT uStyle)
	{
			...
			if( pt.x >= rc.right || *pstrText == _T('\n') || bLineEnd ) {
				...
				if( !bLineDraw ) {
					if( bInLink && iLinkIndex < nLinkRects ) {
						::SetRect(&prcLinks[iLinkIndex++], ptLinkStart.x, ptLinkStart.y, DUI_MIN(pt.x, rc.right), pt.y + cyLine);
					...
					case _T('a'):  // Link
						{
							...
							if( pFontInfo->bUnderline == false ) {
								...
								cyLine = DUI_MAX(cyLine, pTm->tmHeight + pTm->tmExternalLeading + (int)aPIndentArray.GetAt(aPIndentArray.GetSize() - 1));
							}
							ptLinkStart = pt;
							bInLink = true;
						}
							...
							if( sName.IsEmpty() ) { // Italic
								...
								if( pImageInfo ) {
									...
									if( pt.x + iWidth > rc.right && pt.x > rc.left && (uStyle & DT_SINGLELINE) == 0 ) {
										...
										cyLine = DUI_MAX(iHeight, cyLine);
										pt.x += iWidth;
										cyMinHeight = pt.y + iHeight;
										cxMaxWidth = DUI_MAX(cxMaxWidth, pt.x);
									}
								}
					...
					case _T('p'):  // Paragraph
						{
							...
							cyLine = DUI_MAX(cyLine, pTm->tmHeight + pTm->tmExternalLeading + cyLineExtra);
						}
						break;
					case _T('r'):  // Raw Text
						{
				...
				case _T('a'):
					{
						if( iLinkIndex < nLinkRects ) {
							if( !bLineDraw ) ::SetRect(&prcLinks[iLinkIndex], ptLinkStart.x, ptLinkStart.y, DUI_MIN(pt.x, rc.right), pt.y + pTm->tmHeight + pTm->tmExternalLeading);
							iLinkIndex++;
						}
						aColorArray.Remove(aColorArray.GetSize() - 1);
						DWORD clrColor = dwTextColor;
		...
		if( (uStyle & DT_CALCRECT) != 0 ) {
			rc.bottom = DUI_MAX(cyMinHeight, pt.y + cyLine);
			rc.right = DUI_MIN(rc.right, cxMaxWidth);
		}
