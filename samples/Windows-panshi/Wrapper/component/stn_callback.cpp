...
{
	bool bRes = NetworkService::Instance().Req2Buf(_taskid, _user_context, _outbuffer, _extend, _error_code, _channel_select);

#ifdef _DEBUG
	int iLen = _outbuffer.Length();
	printf("StnCallBack::Req2Buf : %d \n", iLen);
	unsigned char* ch = (unsigned char*)_outbuffer.Ptr(0);
...
{
	int iRes = NetworkService::Instance().Buf2Resp(_taskid, _user_context, _inbuffer, _extend, _error_code, _channel_select);

#ifdef _DEBUG
	int iLen = _inbuffer.Length();
	printf("StnCallBack::Buf2Resp : %d \n", iLen);
	unsigned char* ch = (unsigned char*)_inbuffer.Ptr(0);
	for (int i = 0; i < iLen; i++)
	{
		printf("%02X ", ch[i]);
	}
